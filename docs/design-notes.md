...
Script bundles remain the primitive for agentic runtimes — anything that spawns an external CLI, reads files, or runs multi-turn tool loops. The repo's `examples/` carries `claude-code` and `lm-studio` starter bundles; LM Studio support is `cp -r examples/scripts/claude-code scripts/lm-studio` and editing the script. For completion-shaped steps, first-party `llm:` steps call providers declared in `llm-providers.yaml` through the AI SDK — no bundle copy required. Kiri stays runtime-blind for **script** steps: it spawns `run.sh`, captures the envelope, and stays out of the way.
